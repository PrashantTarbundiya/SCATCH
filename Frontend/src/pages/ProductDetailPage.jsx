...
const ProductDetailPage = () => {
  ...
  if (error && !product) return <div className="w-full min-h-screen flex flex-col items-center justify-center pt-20 bg-gray-50 dark:bg-gradient-to-br dark:from-[#0F0A1E] dark:via-[#1A1333] dark:to-[#0F0A1E]"><p className="text-red-600 dark:text-red-400 text-xl mb-4">Error: {error}</p><button onClick={() => navigate('/shop')} className="px-6 py-2 bg-blue-600 dark:bg-purple-600 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-purple-700 transition-colors">Back to Shop</button></div>;
  if (!product) return <div className="w-full min-h-screen flex items-center justify-center pt-20 bg-gray-50 dark:bg-gradient-to-br dark:from-[#0F0A1E] dark:via-[#1A1333] dark:to-[#0F0A1E]"><p className="text-gray-700 dark:text-purple-200 text-lg">Product not found.</p></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gradient-to-br dark:from-[#0F0A1E] dark:via-[#1A1333] dark:to-[#0F0A1E] pt-24 pb-12 px-4 md:px-8 lg:px-16">
      ...
      <div className="bg-white/80 dark:bg-[#1E1538]/60 backdrop-blur-xl border border-purple-500/20 p-4 md:p-6 rounded-lg shadow-xl w-full mx-auto">
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="border dark:border-gray-700 p-4 rounded-lg flex flex-col justify-between">
            <div>
              ...
              <div className="flex items-center mb-2">
                ...
                <span className="ml-2 text-xs text-gray-600 dark:text-gray-600 dark:text-purple-300">({(product.averageRating || 0).toFixed(1)} rating){product.ratings && product.ratings.length > 0 && <span className="ml-1">({product.ratings.length} reviews)</span>}</span>
              </div>
              <div className="mb-3">
                <span className="text-xl font-bold text-blue-600 dark:text-blue-400">₹{(product.price - (product.discount || 0)).toFixed(2)}</span>
                {product.discount > 0 && <span className="ml-2 text-xs text-gray-600 dark:text-purple-300 line-through">₹{product.price.toFixed(2)}</span>}
              </div>
            ...
            <div className="flex items-center gap-2 mt-auto">
                ...
                className={`p-2 rounded-lg transition-colors duration-300
                            ${product && product.quantity > 0
                              ? isProductInWishlist(product._id)
                                ...
                                : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-purple-100 hover:bg-gray-300 dark:hover:bg-gray-600'
                              : 'bg-gray-100 dark:bg-white/80 dark:bg-[#1E1538]/60 backdrop-blur-xl text-gray-400 dark:text-gray-500 cursor-not-allowed'}`}
              ...
              <button
                ...
                className={`flex-grow text-white font-semibold py-2 px-3 rounded-lg transition-colors duration-300 flex items-center justify-center gap-2 ${product && product.quantity > 0 ? 'bg-blue-600 dark:bg-purple-600 hover:bg-blue-700 dark:hover:bg-purple-700' : 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed'}`}
          ...
          {hasPurchasedProduct && currentUser ? (
            <div id="review-form-section-detailpage" className="border dark:border-gray-700 p-4 rounded-lg flex flex-col justify-between">
              <div>
                <h4 className="text-md font-semibold text-gray-700 text-gray-900 dark:text-purple-100 mb-2">{editingReview ? 'Edit Your Review:' : 'Write a Review:'}</h4>
                <div className="flex items-center mb-2">
                  <span className="mr-2 text-sm text-gray-600 text-gray-700 dark:text-gray-700 dark:text-purple-200">Your Rating:</span>
                ...
                <textarea value={reviewText} onChange={(e) => setReviewText(e.target.value)} placeholder="Share your thoughts about the product..." className="w-full p-2 border border-purple-500/30 rounded-md mb-2 bg-white dark:bg-[#2A1F47] text-gray-900 dark:text-purple-100 placeholder-gray-400 dark:placeholder-purple-300/50 focus:ring-2 focus:ring-purple-500 focus:border-transparent" rows="2" disabled={isSubmittingReview}></textarea>
                <div className="mb-2">
                  <label className="block text-xs font-medium text-gray-700 dark:text-purple-200 mb-1">{editingReview && editingReview.reviewImage?.length > 0 && reviewImagePreviews?.length > 0 && reviewImagePreviews.every(p => typeof p === 'string' && p.startsWith('http')) ? "Current images will be kept." : "Upload images (up to 5):"}</label>
                  <input type="file" accept="image/*" multiple onChange={handleImageChange} className="block w-full text-xs text-gray-600 dark:text-purple-300 file:mr-2 file:py-1 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-blue-50 dark:file:bg-blue-900 file:text-blue-700 dark:file:text-blue-300 hover:file:bg-blue-100 dark:hover:file:bg-blue-800 disabled:opacity-50" disabled={isSubmittingReview} />
              ...
              <div className="flex gap-2 mt-auto">
                <button onClick={handleReviewSubmit} disabled={isSubmittingReview || userRating === 0} className="flex-1 bg-green-500 dark:bg-green-600 hover:bg-green-600 dark:hover:bg-green-700 text-white font-semibold py-2 px-3 rounded-lg transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed">{isSubmittingReview ? 'Submitting...' : (editingReview ? 'Update Review' : 'Submit Review')}</button>
                {editingReview && (<button onClick={() => { setEditingReview(null); setUserRating(0); setReviewText(''); setReviewImages([]); setReviewImagePreviews([]); }} disabled={isSubmittingReview} className="bg-gray-300 dark:bg-[#2A1F47] hover:bg-gray-400 text-gray-800 font-semibold py-2 px-3 rounded-lg transition-colors">Cancel Edit</button>)}
              </div>
            </div>
          ) : (
            <div className="border dark:border-gray-700 p-4 rounded-lg flex items-center justify-center h-full"><p className="text-sm text-gray-600 dark:text-purple-300 text-center">{currentUser ? "You must purchase this product to write a review." : "Please log in to write a review."}</p></div>
          )}
        </div>
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center mb-1 justify-between">
                      <div className="flex items-center">
                        ...
                        <span className="ml-2 text-xs font-semibold text-gray-700 text-gray-900 dark:text-gray-900 dark:text-purple-100">{review.user ? (review.user.fullname || review.user.username || `User...${review.user._id?.toString().slice(-4) || 'Unknown'}`) : 'Anonymous'}</span>
                      </div>
                    ...
                    <p className="text-xs text-gray-600 dark:text-purple-300 mb-1">{new Date(review.createdAt).toLocaleDateString()}</p>
                    {review.reviewText && <p className="text-gray-700 dark:text-purple-200 text-sm">{review.reviewText}</p>}
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-600 dark:text-gray-600 dark:text-purple-300">No reviews yet. Be the first to review!</p>
          )}
        </div>
